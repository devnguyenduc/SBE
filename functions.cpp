...
complex<double> E(int n, double t, complex<double> *nsolutions)
{
	...
	for (int l=0; l<(N/2); l++)
	{
		sum += g(n/2 + 1,l+1) * 2.0 *nsolutions[2*l];
	}
	return phase*sum;
...
complex<double> Omega(int n, double t, complex<double> *nsolutions)
{
	...
	for(int l=0;l<(N/2);l++)
	{
		sum += g( (n+1)/2 + 1,l+1)*nsolutions[2*l+1];
	}
...
{
	return ( (-i_/hbar) * ( (n/2.0) * var_epsilon - 30.0 - E(n, t, nsolutions) ) * nsolutions[n] + i_*(1.0-2.0*nsolutions[n-1]) * Omega(n,t,nsolutions) - nsolutions[n] / dephasing_time);
}
//define function fn
